...
from module import save_log_to_folder
    ...
    with sess.as_default():
         ...
         for epoch in range(setting.epoch_num):
            for i in range(int(len(traindata['L'])/setting.batch_size)):
                ...
                #temp, step, loss_, kl_loss,kl2_loss = sess.run([train_op, global_step, m.total_loss, m.kl_loss,m.kl2_loss], feed_dict)
                temp, step, loss_, kl_loss = sess.run([train_op, global_step, m.total_loss, m.kl_loss], feed_dict)
                if step>=50 and step%10==0:
                    time_str = datetime.datetime.now().isoformat()
                    #tempstr = "{}: step {}, softmax_loss {:g}, kl_loss {:g},kl2_loss {:g}".format(time_str, step, loss_, kl_loss,kl2_loss)
                    tempstr = "{}: step {}, softmax_loss {:g}, kl_loss {:g}".format(time_str, step, loss_, kl_loss)
                    save_log_to_folder(log_name,tempstr)
                    print (tempstr)
    ...
    with sess.as_default():
        ...
        for model_iter in testlist:
            ...
            for i in range(int(len(testdata['L'])/setting.batch_size)):
                ...
                feed_dict[mtest.pretrained_output] = list(np.zeros((32,275,304)))
            ...
            tempstr = "model_iter:{}, f1 score:{}, accuracy score:{}".format(model_iter, f1, accuracy)
            save_log_to_folder(testLog_name,tempstr)
            print ('model_iter:',model_iter)
            print ('f1 score:',f1)
            print ('accuracy score:',accuracy)

        save_log_to_folder(testLog_name,'----------------------------')
        tempstr = "best model_iter:{}, best f1 score:{}, best accuracy score:{}".format(best_model_iter, best_model_f1, best_model_acc)
        save_log_to_folder(testLog_name,tempstr)
        print ('----------------------------')
