...
class Dataset:
    def __init__(self):
        self.config = Config()
        self.iter_num = 0
        self.label_set = ['hap','ang','sad','neu','fru','exc','sur']
        self.visual = pickle.load(open('../data/iemocap/processed_visual_dict.pkl', 'rb'))
        self.audio = pickle.load(open('../data/iemocap/audio_dict.pkl', 'rb'))
        self.text = pickle.load(open('../data/iemocap/text_emb.pkl', 'rb'))
        self.label = pickle.load(open('../data/iemocap/label_dict.pkl', 'rb'))
...
#four (happy, angry, sad, and neutral) and seven classes (happy, angry,sad, neutral, frustrated, excited, and surprised).
    def setdata(self, train_size):
        ...
        for i in range(len(self.keys)):
            cur_id = self.keys[i]
            tmp_label = self.label[cur_id]
            if tmp_label in ['hap','ang','sad','neu','fru','exc','sur']:
                if i < train_size:
                    ...
                    traindata['L'].append(self.label_set.index(tmp_label))
                    ...
                    testdata['L'].append(self.label_set.index(tmp_label))
                    testdata['F'].append(self.flag[cur_id])
        return traindata, testdata


    def cal_count(self):
        a= {'neu':0, 'xxx':0, 'fru':0, 'ang':0, 'sad':0, 'hap':0, 'exc':0, 'sur':0, 'oth':0, 'fea':0, 'dis':0}
        for i in range(len(self.keys)):
            a[self.label[self.keys[i]]] += 1
...
if __name__ == '__main__':
    ...
    traindata, testdata = data.setdata(8000)
#    print (traindata['A'][:2])
